...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    ImRect bb = ImRect(bb_frame.Min + ImVec2(0, 10), bb_frame.Max - ImVec2(0, 10));
    ...
    // Render
    window->DrawList->AddRectFilled(bb.Min, bb.Max, gui->get_clr(clr->scrollbar_background), window->WindowRounding, flags);
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
        grab_rect = ImRect(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm), bb.Min.y, ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) + grab_h_pixels, bb.Max.y);
    else
        grab_rect = ImRect(bb.Min.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) - 2, bb.Max.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) + grab_h_pixels + 2);
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, gui->get_clr(clr->accent), style.ScrollbarRounding);
...
template float ImGui::ScaleRatioFromValueT<unsigned int, int, float>(int, unsigned int, unsigned int, unsigned int, bool, float, float);
template unsigned int ImGui::ScaleValueFromRatioT<unsigned int, int, float>(int, float, unsigned int, unsigned int, bool, float, float);
template float ImGui::ScaleRatioFromValueT<unsigned long long, long long, double>(int, unsigned long long, unsigned long long, unsigned long long, bool, float, float);
template unsigned long long ImGui::ScaleValueFromRatioT<unsigned long long, long long, double>(int, float, unsigned long long, unsigned long long, bool, float, float);
